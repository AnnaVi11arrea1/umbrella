...
gmaps_data = HTTP.get(gmaps_url) # gets the dat
...
twelve_hour_prediction = hourly_data_array[1..12] # 12 hours

precip_prob_threshold = 0.10
any_precipitation = false

precip_array = [] # fpr ascii chart

twelve_hour_prediction.each_with_index do |hour_hash, index|
  precip_prob = hour_hash.fetch("precipProbability")
  precip_array.push([index + 1, (precip_prob * 100).round]) # push the array to the ascii chart
...
#ascii chart
puts AsciiCharts::Cartesian.new(precip_array).draw #draw the array
